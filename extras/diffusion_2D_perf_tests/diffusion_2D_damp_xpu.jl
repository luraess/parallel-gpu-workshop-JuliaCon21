# 2D nonlinear diffusion XPU implicit solver with acceleration (perftests)
const USE_GPU = true
...
@views function diffusion_2D_damp_xpu(; do_visu=true, save_fig=false)
    ...
    if do_save open("./output/out_diffusion_2D_damp_xpu.txt","a") do io; println(io, "$(nx) $(ny) $(t_toc) $(T_eff)") end end
    return
end

diffusion_2D_damp_xpu(; do_visu=do_visu)
